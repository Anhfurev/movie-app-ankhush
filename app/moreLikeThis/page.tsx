...
const page = async ({ searchParams }: moreLikeThisProps) => {
  ...
  const similarMovies: movieResponseType = await getMoviesBySame1(id, page);
  const MovieDetail: movieResponseType = await getMovieDetail(id);
  const url = `/moreLikeThis?id=${id}`;

  return (
    <div className="sm:w-[1280px] w-auto flex justify-center flex-col m-auto">
      <h1 className="text-[24px] font-bold flex justify-start sm:mt-20 sm:ml-0 ml-5 mt-14">{`"${MovieDetail.title}" Same movies`}</h1>
